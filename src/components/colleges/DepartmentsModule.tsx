    ...
    <div className="space-y-4" dir="rtl">
      ...
      <Card>
        <CardContent className="pt-6">
          <Table>
            <TableHeader><TableRow><TableHead>القسم</TableHead><TableHead>الكود</TableHead><TableHead>الإجراءات</TableHead></TableRow></TableHeader>
            <TableBody>
              {departments.length === 0 ? <TableRow><TableCell colSpan={3} className="text-center py-4">لا توجد أقسام</TableCell></TableRow> : departments.map((dept) => (
                <TableRow key={dept.department_id} className={cn("cursor-pointer hover:bg-muted/50", selectedDepartment?.department_id === dept.department_id && "bg-muted/70")} onClick={() => setSelectedDepartment(dept)}>
                  <TableCell>{dept.department_name}</TableCell>
                  <TableCell>{dept.department_code}</TableCell>
                  <TableCell><div className="flex gap-2"><Button size="sm" variant="ghost" onClick={(e) => {e.stopPropagation(); openDialog(dept)}}><Edit className="w-4 h-4" /></Button><Button size="sm" variant="destructive" onClick={(e) => {e.stopPropagation(); handleDelete(dept.department_id)}}><Trash2 className="w-4 h-4" /></Button></div></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      {selectedDepartment && (
        <Card className="mt-6 bg-slate-50/50">
          <CardHeader><div className="flex justify-between"><CardTitle>البرامج - {selectedDepartment.department_name}</CardTitle><Button size="sm" onClick={handleAddProgram}><Plus className="w-4 h-4" /></Button></div></CardHeader>
          <CardContent>
            ...
            {selectedProgram && (
                <div className="animate-in fade-in slide-in-from-top-4 mb-6">
                    <div className="flex justify-between items-center mb-2"><h3 className="font-semibold">المستويات</h3><Button size="sm" variant="outline" onClick={handleAddLevel}><Plus className="w-3 h-3" /></Button></div>
            ...
            {selectedLevel && (
                 <div className="mb-6 animate-in fade-in slide-in-from-top-4">
                    <div className="flex justify-between items-center mb-2"><h3 className="font-semibold">الفصول (الترم)</h3><Button size="sm" variant="outline" onClick={handleAddTerm}><Plus className="w-3 h-3" /></Button></div>
